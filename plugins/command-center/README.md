...
**v0.18.0 — Praxis-Audit-Paket (Befunde vom Galant-Firmen-PC, 21.07.).** Vier Baustellen aus dem echten Betrieb: **(1) Engine-Target-Guard:** `apply.py` kopiert relative Ziele nur noch unterhalb erlaubter Ausgabe-Wurzeln (`_ausgang/` immer; dazu Ordner, die in `_firma/config/*.json` unter einem Pfad-Feld wie `…_ordner`/`…_base`/`…_path`/`output_roots`/`kontrolle` stehen; `_firma/` und `_eingang/` nie) — ein Config-SCHLÜSSEL als target („buchhaltung") wird als `skipped-unallowed-target` übersprungen statt still Ordner im Workspace-Root anzulegen (der 21.07.-Vorfall: 30 Fehlablagen in 3 Läufen). **(2) Offene-Rechnungen-Liste:** receipt-filing pflegt `<ablage_ordner>/Offene-Rechnungen.md` — jede zahlungspflichtige Rechnung ohne SEPA-Einzug als Checkbox-Zeile (Dedupe über die Beleg-Nr, Mahnung ergänzt die bestehende Zeile samt Frist; abhaken tut nur die Firma, der Skill nie; LS/LF/GS/Avis/SEPA-Einzüge kommen nicht drauf). **(3) Sichtbarkeit:** Signale mit `severity:"folgenreich"` (Mahnfrist, Steuer-Widerspruch …) landen zusätzlich als `WICHTIG:`-Zeile in `Kontrolle-Notizen.md`; bestätigte neue Lieferanten/Baustellen schreibt der Skill selbst in die Stammdaten (inkl. SEPA-Ableitung) statt sie nur zu loggen. **(4) Praxis-Fixes:** Text-PDFs per Textextraktion statt Vision (schneller/billiger, Vision nur für Scans); Step 0 unterscheidet „Ordner nicht verbunden" (→ Verbinden-Hinweis) von „nicht eingerichtet" (→ Onboarding); Windows-Referenzpfad-Robustheit (`.remote-plugins`-Mount → Datei per Namen suchen); Namens-Schemas für GS/Mahnung/Bescheid/Avis als Defaults + „ein Lieferant, ein Name".

**v0.17.0 — receipt-filing: flache Ablage, keine Unterordner (Firmentest-Feedback).** Alle benannten Belege landen in EINEM Ablage-Ordner (`ablage_ordner`, Default `_ausgang/belege`) — die KI legt **nie** Unterordner an (keine Monats-/Jahres-/Entity-/Kontrolle-Ordner; einzige erlaubte Anlage ist der Ablage-Ordner selbst). Unklare Belege liegen im selben Ordner mit Dateinamen-Präfix `PRÜFEN - ` + Zeile in `Kontrolle-Notizen.md`; Kontrolle = Umbenennen im Ordner (oder „Beleg X passt" im Chat → Skill entfernt das Präfix). Das bisherige Multi-Target-Routing (Buchhaltung/Monatsordner, Offene Rechnungen, Lager, Baustelle, Kontrolle-Ordner) ist nur noch Opt-in via `ablage_modus: "routing"` — eine ältere Config mit `targets` ohne diesen Key wird flach abgelegt. Rückfall-Freigabe `"ablage": "review"` unverändert.
