...
**Ausführung nach Prozess-Vertrag:** Fotos, Tagesberichte und Rechnungen bleiben **vorbereitet** — nichts wird kopiert, verschoben oder finalisiert; Freigabe über das Review-Board. **Belege (`receipt-filing`) laufen als Direktablage — flach:** die Beleg-Queue bekommt `runid` mit Zeit-Suffix, ALLE Aktionen zielen auf den EINEN `ablage_ordner` (Default `_ausgang/belege`, nie Unterordner anlegen; `targets` sind immer **Pfade**, nie Config-Schlüssel wie „buchhaltung" — die Engine überspringt solche Ziele; `prüfen`-Posten mit Dateinamen-Präfix `PRÜFEN - `), und direkt nach dem Schreiben wird `python3 _firma/apply.py <workspace_root> approve-run <runid>` ausgeführt — die Belege sind damit schon geparkt, bevor das Board öffnet; danach Offene-Rechnungen-Liste und ggf. Kontrolle-Notizen pflegen (Details: `skills/receipt-filing/SKILL.md` Step 3 + `reference/rules.md` §Offene-Rechnungen-Liste; bei `"ablage": "review"` in der Beleg-Config stattdessen parken wie die anderen Prozesse).
...
Wenn alle Queues geschrieben (und die Beleg-Queue ausgeführt) sind, gib **eine** kurze Klartext-Nachricht: wie viele Belege **schon direkt abgelegt** wurden (davon wie viele mit `PRÜFEN - `-Markierung), wie viele Fotos / Berichte vorbereitet wurden, welche Dubletten/Notizen aussortiert wurden — und dass die Freigabe der übrigen Prozesse jetzt **Prozess für Prozess** läuft. (Eine echte Push-Benachrichtigung gibt es nicht; diese Nachricht ist der Hinweis.)
...
- Belege sind **direkt abgelegt** (Journal + Activity `status: done`; flach im Ablage-Ordner, Unklares mit `PRÜFEN - `-Präfix); für die übrigen Prozesse je eine Review-Queue + `status: prepared`, dort wurde nichts bewegt.
