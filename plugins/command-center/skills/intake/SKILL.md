...
Für die verbleibenden NEUEN Dateien zuerst **Dubletten erkennen**: bilde je Datei eine Inhalts-Prüfsumme (`md5sum`). Gleiche Prüfsumme = dieselbe Datei (auch über Ordner/Endungen hinweg) → nur **eine** verarbeiten, die übrigen als Dublette markieren. Prüfe außerdem **Magic-Bytes gegen Endung** (`file -b`): eine als `.pdf` benannte JPEG ist ein Bild, kein PDF — nach Inhalt behandeln, nie nach Endung.
