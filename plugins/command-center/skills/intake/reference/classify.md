...
4. **Handschriftlich ausgefüllter Bericht-Vordruck** (Montage-/Servicebericht/Stundenzettel: Datum-Spalten, Team-Zeile, Stunden, Unterschriften) → **Doppel-Route**:
   - `bericht_scans: an` in `_firma/config/photo-sorting.json` → **`photo-sorting` (Modus B)** zur Archiv-Umbenennung (`JJJJ KWnn BV V.Nachname …`, siehe `skills/photo-sorting/reference/bericht-scans.md`) — **immer**.
   - Enthält er Stunden zur Abrechnung → **zusätzlich `invoicing`** (dieselbe Quelldatei, zwei Posten in zwei Queues; ob abgerechnet werden soll, einmal gebündelt fragen, wenn es nicht aus dem Auftrag klar ist).
   - `bericht_scans: aus` → wie bisher nur **`invoicing`**.
...
2. **Stundenzettel / Montagebericht / Wochenstunden zur Abrechnung** — Marker: Wochenübersicht mit Arbeiter-Stunden, Tarif/Stufen, Spesen, KW → **`invoicing`**. Ist die „PDF" in Wahrheit ein **Scan eines handschriftlichen Vordrucks**, gilt die Doppel-Route aus A4 (photo-sorting Modus B + ggf. invoicing).
...
## daily-report vs. invoicing vs. photo-sorting Modus B (Abgrenzung)
...
- **photo-sorting Modus B**: archiviert den **Scan selbst** — umbenannte Kopie (`JJJJ KWnn BV V.Nachname …`) im KWnn-Ordner. Kein Ersatz für invoicing, sondern parallel dazu (Doppel-Route A4).
Im Zweifel: enthält das Dokument Tätigkeits-Beschreibungen pro Tag → daily-report; enthält es v. a. Stunden/Beträge zur Abrechnung → invoicing; ist es ein abfotografierter handschriftlicher Vordruck → zusätzlich Modus B (Archiv).
