...
1. **Volle editierbare Karte** (visualize `show_widget`; `read_me` `interactive` einmal vorab) — siehe `${CLAUDE_PLUGIN_ROOT}/skills/review-board/reference/board-ui.md`. Sie zeigt **alle** Infos und macht sie **bearbeitbar**:
   ...
   - **Pflicht-Bestätigungen zuerst und unübersehbar:** trägt der Posten `bestaetigen`-Einträge (offene Rückfragen: Hotelbetrag, Fahrt-Aufteilung bei Zeitüberlappung, schwer lesbare Zahl), rendere sie als eigenen, hervorgehobenen Frage-Block ÜBER den editierbaren Feldern — je Eintrag `frage`, ggf. `quelle_auszug` (was wirklich im Dokument steht) und ein Antwortfeld mit eigenem „Bestätigen"-Button. Kein Fließtext im `reason` — genau davor waren diese Fragen bisher unsichtbar. Die Engine wendet solche Posten ohnehin nicht an (`needs-confirmation`).
...
## Step 5 — Intents
- **Bestätigen (Pflichtfeld)**: `bestätige <runid> <id>: <feld>=<wert>; …` → den passenden `bestaetigen`-Eintrag in der Queue patchen (`wert` setzen, `bestaetigt: true`), den Wert in `values.<feld>` spiegeln, `rechecked` setzen. Bei Geld-Werten (invoicing: `hotel_betrag`, `fahrt_h_*`, `km_*`) danach **`compute.ts` mit dem bestätigten Wert neu laufen lassen** und Entwurf/`values`/Vorschau aktualisieren — niemals Summen von Hand nachziehen. Round-Trip-Pflicht wie beim Bearbeiten; Karte neu rendern. Erst wenn alle Einträge des Postens beantwortet sind, wendet die Engine ihn an (`approve`/`approve-run`/`approve-safe`/`manual-confirm` liefern sonst `needs-confirmation`).
...
- **Prozess freigeben (Hauptklick)**: `freigeben prozess <process>` → **alle verbliebenen** Posten dieses Prozesses in EINEM Engine-Lauf speichern (Step 6, `approve-run`), Queue archivieren, dann **sofort den nächsten offenen Prozess** rendern (zurück zu Step 3). Posten mit offener Pflicht-Bestätigung überspringt die Engine (`needs-confirmation`) — sie bleiben in der Queue; nenne sie danach ausdrücklich mit ihren offenen Fragen statt sie als erledigt zu melden. Gibt es keinen offenen Prozess mehr → Abschluss-Zusammenfassung.
