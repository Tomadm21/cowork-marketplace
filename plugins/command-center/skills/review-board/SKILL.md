...
**Fail-fast statt Retry-Schleife.** Schlägt ein Kopiervorgang fehl (Timeout, `error` im Engine-Ergebnis), **nicht** automatisch wiederholen: genau EIN Wiederholungsversuch ist erlaubt; danach dem Nutzer sofort den „Manuell erledigt"-Pfad anbieten (Quelle + Zielpfad nennen, Nutzer kopiert selbst, dann `manual-confirm`). Wiederholte automatische Kopierversuche auf ein strukturell langsames Ziel verbrennen nur Sitzungszeit.
